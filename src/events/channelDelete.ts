...
function getChannelTypeName(type: ChannelType): string {
    switch (type) {
        ...
        case ChannelType.AnnouncementThread: return 'AnnouncementThread';
        case ChannelType.PublicThread: return 'PublicThread';
        case ChannelType.PrivateThread: return 'PrivateThread';
        case ChannelType.GuildStageVoice: return 'StageVoice';
        ...
        case ChannelType.GroupDM: return 'GroupDM';
        default: return `Unknown (${String(type)})`;
    }
}
  ...
  async execute(channel: DMChannel | GuildChannel) {
    ...
    try {
        ...
        if (deleteLog) {
            ...
            logger.warn(`Could not find matching Audit Log for ${eventType} on channel ${channelId}. Executor info might be missing.`);
        }
    } catch (error) {
        logger.error(`Failed to fetch Audit Logs for ${eventType} on channel ${channelId}:`, error);
    }
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event for channel ${channelId} (${dataToStore.channelName}) in guild ${guildId}`);
    } catch (error) {
