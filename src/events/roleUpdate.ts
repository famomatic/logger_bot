...
function formatChange(change: AuditLogChange): string {
    ...
    if (change.key === 'permissions') {
        oldValue = new PermissionsBitField(BigInt(change.old as string | number)).toArray().join(', ') || '없음';
        newValue = new PermissionsBitField(BigInt(change.new as string | number)).toArray().join(', ') || '없음';
    } else if (change.key === 'color' && typeof change.old === 'number' && typeof change.new === 'number') {
        // 색상 변경은 숫자에서 HEX 코드로 변환
        oldValue = `#${change.old.toString(16).padStart(6, '0')}`;
        newValue = `#${change.new.toString(16).padStart(6, '0')}`;
    } else {
        oldValue = typeof change.old === 'object' && change.old !== null ? JSON.stringify(change.old) : String(change.old ?? '없음');
        newValue = typeof change.new === 'object' && change.new !== null ? JSON.stringify(change.new) : String(change.new ?? '없음');
    }
  ...
  async execute(oldRole: Role, newRole: Role) {
    ...
    try {
        ...
        if (updateLog) {
            ...
            logger.warn(`Could not find exact Audit Log entry for ${eventType} (role ${targetId}) in guild ${guildId}. Executor and precise changes might be missing.`);
            // Audit Log를 못 찾으면 직접 비교 (단, 어떤 변경이 있었는지 정확히 모를 수 있음)
            const detectedChanges: string[] = [];
            if (oldRole.name !== newRole.name) detectedChanges.push(`이름: '${oldRole.name}' -> '${newRole.name}'`);
            if (oldRole.hexColor !== newRole.hexColor) detectedChanges.push(`색상: '${oldRole.hexColor}' -> '${newRole.hexColor}'`);
            if (oldRole.permissions.bitfield !== newRole.permissions.bitfield) detectedChanges.push(`권한 변경됨 (상세 내역은 Audit Log 확인 필요)`);
            if (oldRole.hoist !== newRole.hoist) detectedChanges.push(`분리 표시: ${oldRole.hoist} -> ${newRole.hoist}`);
            if (oldRole.mentionable !== newRole.mentionable) detectedChanges.push(`언급 가능: ${oldRole.mentionable} -> ${newRole.mentionable}`);
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event for role ${newRole.name} (${targetId}) in guild ${guildId}`);
    } catch (error) {
