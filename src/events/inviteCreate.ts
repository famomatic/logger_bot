...
const event = {
  ...
  async execute(invite: Invite) {
    ...
    if (!invite.guild || !invite.inviter || !invite.channel)
        return;
    ...
    if (!guild) {
        logger.warn(`Could not find guild ${guildId} in cache for invite ${invite.code}. Cannot fetch audit logs.`);
    }
    ...
    if (guild) {
        try {
            ...
            const createLog = fetchedLogs.entries.find(entry =>
                entry.changes?.some((c: AuditLogChange) => c.key === 'code' && c.new === inviteCode)
            );
            ...
            if (createLog && Math.abs(Date.now() - createLog.createdTimestamp) < 5000) {
                ...
                logger.warn(`Could not confirm executor for ${eventType} (code ${inviteCode}) in guild ${guildId} via Audit Log. Executor might be missing.`);
            }
        } catch (error) {
