import { Events, Message, PartialMessage, AuditLogEvent, User, GuildAuditLogsEntry } from 'discord.js';
...
async function processBuffer() {
  ...
  for (const [gid, msgs] of guildMap) {
    ...
    try {
      const fetchedLogs = await guild.fetchAuditLogs({ limit: 5, type: AuditLogEvent.MessageDelete });
...
async function handleDelete(message: Message | PartialMessage, fetchedEntries: GuildAuditLogsEntry[] | null) {
  ...
  if (!guildId || !isGuildAuthorized(guildId)) {
    logger.warn(`Unauthorized ${eventType} event logging on guild ${guildId ?? 'unknown'} skipped`);
    return;
  }
  ...
  if (message.partial) {
    ...
    if (fetchedEntries && author && message.guild) {
      const deleteLog = fetchedEntries.find(entry =>
        entry.extra && typeof entry.extra === 'object' && 'channel' in entry.extra &&
  ...
  try {
    ...
    if (logged) {
      logger.debug(`Logged new ${eventType} for message ${messageId} in channel ${channelId}`);
    }
  } catch (error) {
    logger.error(`Error occurred while attempting to log ${eventType} for message ${messageId}:`, error);
  }
}
