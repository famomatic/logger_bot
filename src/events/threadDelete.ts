  ...
  async execute(thread: ThreadChannel) {
    ...
    try {
        ...
        if (deleteLog && deleteLog.target?.id === targetId && Math.abs(Date.now() - deleteLog.createdTimestamp) < 5000) {
            ...
            logger.warn(`Could not confirm executor for ${eventType} (thread ${targetId}) in guild ${guildId} via Audit Log.`);
        }
    } catch (error) {
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event for thread ${deletedThreadInfo.name} (${targetId}) in guild ${guildId}`);
    } catch (error) {
