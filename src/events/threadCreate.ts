    ...
    async execute(thread: ThreadChannel, newlyCreated: boolean) {
        ...
        try {
            ...
            if (
                ...
                logger.warn(
                    `Could not confirm executor for ${eventType} (thread ${targetId}) in guild ${guildId} via Audit Log. Assuming owner is executor.`,
                );
            }
        } catch (error) {
        ...
        try {
            ...
            logger.debug(
                `Logged ${eventType} event for thread ${thread.name} (${targetId}) in guild ${guildId}`,
            );
        } catch (error) {
            logger.error(
