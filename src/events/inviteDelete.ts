...
const event = {
    ...
    async execute(invite: Invite) {
        ...
        if (!guild) {
            logger.warn(
                `Could not find guild ${guildId} in cache for deleted invite ${invite.code}. Cannot fetch audit logs.`,
            );
        ...
        if (guild) {
            try {
                ...
                const deleteLog = fetchedLogs.entries.find(
                    (entry) =>
                        entry.changes.some(
                            (c: AuditLogChange) => c.key === 'code' && c.old === inviteCode,
                        ), // 이전 값(old)과 비교
                );
                ...
                if (deleteLog && Math.abs(Date.now() - deleteLog.createdTimestamp) < 5000) {
                    ...
                    logger.warn(
                        `Could not confirm executor for ${eventType} (code ${inviteCode}) in guild ${guildId} via Audit Log. Invite might have expired.`,
                    );
                }
            } catch (error) {
                logger.error(
