...
const event = {
  ...
  async execute(oldMessage: Message | PartialMessage, newMessage: Message | PartialMessage) {
    ...
    const timestamp = newMessage.editedTimestamp ? new Date(newMessage.editedTimestamp) : new Date(); // Date 객체로 변환
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} for message ${messageId} by ${author.tag} in channel ${channelId}`);
    } catch (error) {
      logger.error(`Error occurred while trying to log ${eventType} for message ${messageId}:`, error);
    }
  },
} as const;
