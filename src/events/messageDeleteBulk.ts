import { Events, Collection, Message, PartialMessage, GuildTextBasedChannel, AuditLogEvent } from 'discord.js';
import { logger } from '../utils/logger.js';
import { logEvent } from '../db/database.js';

const event = {
  name: Events.MessageBulkDelete,
  async execute(messages: Collection<string, Message | PartialMessage>, channel: GuildTextBasedChannel) {
    ...
    if (deletedCount === 0) {
        logger.warn(`Received ${eventType} event with 0 messages in channel ${channelId} of guild ${guildId}. Skipping log.`);
    ...
    try {
        ...
        if (deleteLog) {
            ...
            logger.warn(`Could not find matching Audit Log entry for ${eventType} (${deletedCount} messages) in channel ${channelId} of guild ${guildId}. Executor info might be missing.`);
        }
    } catch (error) {
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event (${deletedCount} messages) in channel ${channelId} of guild ${guildId}`);
    } catch (error) {
      logger.error(`Error occurred while trying to log ${eventType} event in channel ${channelId}:`, error);
    }
  },
} as const;
