  ...
  async execute(member: GuildMember | PartialGuildMember) {
    ...
    if (targetUser) {
        try {
            ...
            logger.error(`Failed to fetch Audit Logs for ${eventType} (user ${targetId}) in guild ${guildId}:`, error);
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event (${action}) for user ${targetUser?.tag ?? targetId} in guild ${guildId}`);
    } catch (error) {
