import { Events, GuildScheduledEvent, GuildScheduledEventEntityType, GuildScheduledEventStatus, AuditLogEvent } from 'discord.js';
...
const event = {
  ...
  async execute(guildScheduledEvent: GuildScheduledEvent) {
    ...
    const timestamp = guildScheduledEvent.createdTimestamp ? new Date(guildScheduledEvent.createdTimestamp) : new Date();
    ...
    try {
        ...
        if (createLog && createLog.target?.id === targetId && Math.abs(Date.now() - createLog.createdTimestamp) < 5000) {
            ...
            logger.warn(`Could not confirm executor for ${eventType} (event ${targetId}) in guild ${guildId} via Audit Log. Assuming creator is executor.`);
        }
    } catch (error) {
    ...
    const dataToStore = {
      ...
      scheduledStartTime: guildScheduledEvent.scheduledStartTimestamp ? new Date(guildScheduledEvent.scheduledStartTimestamp).toISOString() : null,
      scheduledEndTime: guildScheduledEvent.scheduledEndTimestamp ? new Date(guildScheduledEvent.scheduledEndTimestamp).toISOString() : null,
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event for scheduled event ${guildScheduledEvent.name} (${targetId}) in guild ${guildId}`);
    } catch (error) {
