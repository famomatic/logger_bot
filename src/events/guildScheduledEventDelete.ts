...
const event = {
  ...
  async execute(guildScheduledEvent: GuildScheduledEvent) {
    ...
    const deletedEventInfo = {
        ...
        createdAt: guildScheduledEvent.createdTimestamp ? new Date(guildScheduledEvent.createdTimestamp).toISOString() : null,
    };
    ...
    try {
        ...
        if (deleteLog && deleteLog.target?.id === targetId && Math.abs(Date.now() - deleteLog.createdTimestamp) < 5000) {
            ...
            logger.warn(`Could not confirm executor for ${eventType} (event ${targetId}) in guild ${guildId} via Audit Log.`);
        }
    } catch (error) {
    ...
    try {
      ...
      logger.debug(`Logged ${eventType} event for scheduled event ${deletedEventInfo.eventName} (${targetId}) in guild ${guildId}`);
    } catch (error) {
