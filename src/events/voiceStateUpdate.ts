...
async function logVoiceEvent(
    ...
    try {
      ...
      if (logged) {
        logger.debug(`Logged ${eventType} event for user ${user.tag} (${userId}) in guild ${guildId}`);
      } else {
        logger.debug(`Skipped logging duplicate ${eventType} for user ${userId} (${user.tag}) in channel ${channelId}`);
      }
    } catch (error) {
      logger.error(`Error occurred while trying to log ${eventType} event for user ${userId} in guild ${guildId}:`, error);
    }
}
  ...
  async execute(oldState: VoiceState, newState: VoiceState) {
    ...
    if (oldChannelId !== newChannelId) {
      if (!oldChannelId && newChannelId) {
        // 참가 (Join)
        await logVoiceEvent(
          'voiceChannelJoin', guild, user, newChannelId, timestamp,
          { channelId: newChannelId, channelName: newChannelName }
        );
      } else if (oldChannelId && !newChannelId) {
        // 퇴장 (Leave)
        await logVoiceEvent(
          'voiceChannelLeave', guild, user, oldChannelId, timestamp,
          { channelId: oldChannelId, channelName: oldChannelName }
        );
      } else if (oldChannelId && newChannelId) {
        // 이동 (Move)
        await logVoiceEvent(
          'voiceChannelMove', guild, user, newChannelId, timestamp,
          {
            oldChannelId: oldChannelId, oldChannelName: oldChannelName,
            newChannelId: newChannelId, newChannelName: newChannelName,
          }
        );
      }
    }
    ...
    if (currentChannelId) {
        ...
        if (oldState.serverMute !== newState.serverMute || oldState.serverDeaf !== newState.serverDeaf) {
            ...
            const eventType = isMuteChange ? 'voiceStateUpdateServerMute' : 'voiceStateUpdateServerDeaf';
            ...
            try {
                ...
                const stateLog = fetchedLogs.entries.find(entry =>
                ...
                logger.error(`Failed to fetch Audit Logs for ${eventType} in guild ${guild.id}:`, error);
            }

            await logVoiceEvent(
                eventType, guild, user, currentChannelId, timestamp,
                {
                    channelId: currentChannelId, channelName: newChannelName,
                    change: changeKey, oldStatus: oldStatus, newStatus: newStatus,
        ...
        if (oldState.selfMute !== newState.selfMute) {
            await logVoiceEvent(
                'voiceStateUpdateSelfMute', guild, user, currentChannelId, timestamp,
                { channelId: currentChannelId, channelName: newChannelName, muted: newState.selfMute }
            );
        }
        if (oldState.selfDeaf !== newState.selfDeaf) {
            await logVoiceEvent(
                'voiceStateUpdateSelfDeaf', guild, user, currentChannelId, timestamp,
                { channelId: currentChannelId, channelName: newChannelName, deafened: newState.selfDeaf }
            );
        }
        ...
        if (oldState.streaming !== newState.streaming) {
            await logVoiceEvent(
                'voiceStateUpdateStreaming', guild, user, currentChannelId, timestamp,
                { channelId: currentChannelId, channelName: newChannelName, streaming: newState.streaming }
            );
        }
        ...
        if (oldState.selfVideo !== newState.selfVideo) {
            await logVoiceEvent(
                'voiceStateUpdateVideo', guild, user, currentChannelId, timestamp,
                { channelId: currentChannelId, channelName: newChannelName, video: newState.selfVideo }
            );
        }
    }
  },
