    ...
    if (!channel.isDMBased() && channel.guild) {
        ...
        try {
            ...
            const pinLog = fetchedLogs.entries
                .filter(entry => 
                    (entry.action === AuditLogEvent.MessagePin || entry.action === AuditLogEvent.MessageUnpin) &&
                    entry.extra && 'channel' in entry.extra && entry.extra.channel?.id === channelId
                )
            ...
            if (pinLog) {
                ...
                targetUserId = (pinLog.target && typeof pinLog.target === 'object' && 'id' in pinLog.target && typeof pinLog.target.id === 'string') ? pinLog.target.id : null;
                // extra에 messageId가 있는지 확인
                messageId = (pinLog.extra && typeof pinLog.extra === 'object' && 'messageId' in pinLog.extra && typeof pinLog.extra.messageId === 'string') ? pinLog.extra.messageId : null;
                actionType = pinLog.action === AuditLogEvent.MessagePin ? 'pin' : 'unpin';
            } else {
                logger.warn(`Could not find Audit Log for ${eventType} in channel ${channelId} around ${timestamp.toISOString()}. Details might be missing.`);
                // Audit Log를 못 찾으면 실행자 등을 알 수 없음
            }
        } catch (error) {
            logger.error(`Failed to fetch Audit Logs for ${eventType} in channel ${channelId}:`, error);
        }
        ...
        try {
          ...
          logger.debug(`Logged ${eventType} event in channel ${channelId} of guild ${guildId}`);
        } catch (error) {
          logger.error(`Error occurred while trying to log ${eventType} event for channel ${channelId}:`, error);
        }
    }
  },
} as const;
