...
async function findGuildUpdateLog(guild: Guild, changeKey: string): Promise<{ executorId: string | null, logTimestamp: Date | null }> {
    try {
        ...
        if (updateLog) {
            return { executorId: updateLog.executor?.id ?? null, logTimestamp: updateLog.createdAt };
        }
    } catch (error) {
        logger.error(`Failed to fetch Audit Logs for GuildUpdate (${changeKey}) in guild ${guild.id}:`, error);
    }
    return { executorId: null, logTimestamp: null };
}

// 헬퍼 함수: 로그 기록 (수정됨)
async function logGuildUpdate(eventType: string, guildId: string, userId: string | null, targetId: string, timestamp: Date, data: Record<string, unknown>) {
    try {
        await logEvent(
            eventType,
            guildId,
        ...
        logger.error(`Error occurred while trying to log ${eventType} event for guild ${guildId}:`, error);
    }
}
...
const event = {
  ...
  async execute(oldGuild: Guild, newGuild: Guild) {
    ...
    if (oldGuild.name !== newGuild.name) {
        ...
        await logGuildUpdate('guildNameUpdate', guildId, executorId, guildId, logTimestamp ?? baseTimestamp, dataToStore);
    }
    ...
    if (oldGuild.icon !== newGuild.icon) {
        ...
        await logGuildUpdate('guildIconUpdate', guildId, executorId, guildId, logTimestamp ?? baseTimestamp, dataToStore);
    }
    ...
    if (oldGuild.ownerId !== newGuild.ownerId) {
        ...
        try {
            const oldOwner = await oldGuild.client.users.fetch(oldGuild.ownerId).catch(() => null);
        ...
        await logGuildUpdate('guildOwnerUpdate', guildId, executorId, guildId, logTimestamp ?? baseTimestamp, dataToStore);
    }
