...
function serializeEmbedWithSnakeCase(embed: Message['embeds'][number]): Record<string, unknown> {
    return {
        title: embed.title,
        description: embed.description,
        url: embed.url,
...
function extractForwardedContentData(message: Message): Record<string, unknown>[] | null {
    const snapshotMessages = Array.from(message.messageSnapshots.values());
    if (snapshotMessages.length > 0) {
        return snapshotMessages.map((snapshot) => ({
            id: snapshot.id,
            ...
            embeds: snapshot.embeds.map((emb) => serializeEmbedWithSnakeCase(emb)),
    ...
    return rawFwMessages.map((origMsg: ForwardedMessage) => {
...
async function resolveReferencedMessageData(
    message: Message,
    client: Client,
    guildId: string,
    ...
    try {
        ...
        const errCode = error instanceof Error ? (error as ErrorWithCode).code : undefined;
...
const event = {
    ...
    async execute(
        ...
        const embeds = message.embeds.map((embed) => serializeEmbedForStorage(embed));
        const forwardedContentData = extractForwardedContentData(message);
        const referencedMessageData = await resolveReferencedMessageData(message, client, guildId);
