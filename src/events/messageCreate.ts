...
const event = {
  ...
  async execute(message: Message, client: Client, legacyCommands: Collection<string, LegacyCommand>) {
    ...
    if (devLevel >= 1 && message.content.startsWith(BOT_PREFIX) && legacyCommands && legacyCommands.size > 0) {
        ...
        if (commandName) {
            ...
            if (command) {
                try {
                    ...
                    logger.info(`Executed legacy command '${commandName}' by dev: ${message.content}`);
                } catch (error) {
                    logger.error(`Error executing legacy command '${commandName}':`, error);
                    await message.reply(`레거시 명령어 실행 중 오류 발생: ${error instanceof Error ? error.message : String(error)}`);
                }
                return; // 레거시 명령어 실행 시 일반 로깅 건너뛰기
            } else {
                logger.debug(`Unknown legacy command '${commandName}' tried by dev: ${commandName} (full: ${message.content})`);
            }
        }
    }
    // --- ------------------- ---
    ...
    if (message.attachments.size > 0) {
        logger.debug(`Processing ${message.attachments.size} attachments for message ${messageId}`);
        for (const attachment of message.attachments.values()) {
            let webdavPath: string | null = null;
            let downloadError: string | null = null;

            if (config.storage.type) { // Assumes type is present if we are here
                try {
                    const fileBuffer = await downloadWithRetry(attachment.url, 3);
                    logger.debug(`Downloaded ${attachment.name ?? 'unnamed'} (${fileBuffer.length} bytes)`);

                    const relativePath = createWebDAVPath(guildId, channelId, messageId, attachment);
                    // Use storageManager to upload
                    webdavPath = await storageManager.upload(relativePath, fileBuffer);

                } catch (error: unknown) {
                    downloadError = error instanceof Error ? error.message : 'Unknown download/upload error';
                    logger.error(`Failed to download/upload attachment ${attachment.id} (${attachment.name ?? 'unnamed'}):`, error);
                }
            }
    ...
    const embeds = message.embeds.map(embed => ({
      ...
      image: embed.image ? { url: embed.image.url, proxyURL: embed.image.proxyURL, height: embed.image.height, width: embed.image.width } : null,
      thumbnail: embed.thumbnail ? { url: embed.thumbnail.url, proxyURL: embed.thumbnail.proxyURL, height: embed.thumbnail.height, width: embed.thumbnail.width } : null,
      video: embed.video ? { url: embed.video.url, proxyURL: embed.video.proxyURL, height: embed.video.height, width: embed.video.width } : null,
      ...
      provider: embed.provider ? { name: embed.provider.name, url: embed.provider.url } : null,
    }));

    // 전달된 메시지 정보 추출 (message.type === 24 인 경우)
    let forwardedContentData: Record<string, unknown>[] | null = null;
    if ((message.type as number) === 24) { // MessageType.Forward equivalent 
        ...
        if (rawFwMessages && Array.isArray(rawFwMessages) && rawFwMessages.length > 0) {
            forwardedContentData = rawFwMessages.map((origMsg: ForwardedMessage) => {
                ...
                const fwdAttachments = (origMsg.attachments ?? []).map((att: ForwardedAttachment) => ({
                    id: att.id,
                ...
                const fwdEmbeds = (origMsg.embeds ?? []).map((emb: ForwardedEmbed) => ({
                    ...
                    color: emb.color,
                    footer: emb.footer ? { text: emb.footer.text, icon_url: emb.footer.icon_url ?? emb.footer.iconURL } : null,
                    image: emb.image ? { url: emb.image.url, proxy_url: emb.image.proxy_url ?? emb.image.proxyURL, height: emb.image.height, width: emb.image.width } : null,
                    thumbnail: emb.thumbnail ? { url: emb.thumbnail.url, proxy_url: emb.thumbnail.proxy_url ?? emb.thumbnail.proxyURL, height: emb.thumbnail.height, width: emb.thumbnail.width } : null,
                    video: emb.video ? { url: emb.video.url, proxy_url: emb.video.proxy_url ?? emb.video.proxyURL, height: emb.video.height, width: emb.video.width } : null,
                    author: emb.author ? { name: emb.author.name, url: emb.author.url, icon_url: emb.author.icon_url ?? emb.author.iconURL } : null,
                    fields: (emb.fields ?? []).map((field: ForwardedEmbedField) => ({ name: field.name, value: field.value, inline: field.inline })),
                ...
                return {
                    ...
                    author: {
                        ...
                        tag: author.username && author.discriminator ? `${author.username}#${author.discriminator}` : author.username, 
                        bot: author.bot,
                    },
    ...
    if (message.reference?.messageId) {
        ...
        if (!refMsg) {
            try {
                const res = await pool.query<DbLogRow>('SELECT data FROM event_logs WHERE guild_id = $1 AND event_type = $2 AND target_id = $3 LIMIT 1', [guildId, 'messageCreate', refId]);
        ...
        if (!referencedMessageData) {
            try {
                if (!refMsg) {
                    ...
                    if (originalChannel && (originalChannel.isTextBased() || originalChannel.isDMBased())) {
                ...
                const errCode = error instanceof AxiosError ? error.code : (error instanceof Error ? (error as { code?: string }).code : undefined);
    ...
    try {
      ...
      logger.error(`Error occurred while trying to log ${eventType} event for message ${messageId}:`, error);
    }
  },
} as const;
