...
function extractTimeoutOption(payload: string, defaultTimeoutMs?: number): { command: string; timeoutMs?: number } {
    ...
    try {
      ...
      child.on('close', code => {
        if (code === 0) {
          ...
          reject(new Error(stderr.trim() || stdout.trim() || 'sudo 비밀번호를 가져오는 데 실패했습니다.'));
        }
      });
...
async function runPtyProcess(
  program: string,
  args: string[],
  options: { env?: NodeJS.ProcessEnv; timeoutMs?: number; sudoPassword?: string; watchForSudo?: boolean },
...
async function runBashCommand(rawCommand: string, options: { timeoutMs?: number; sudoPassword?: string }): Promise<CommandResult> {
  let command = rawCommand.trim();
  const containsSudo = /\bsudo\b/.test(command);

  if (containsSudo) {
    if (!options.sudoPassword) {
      throw new Error('sudo 명령을 실행하려면 SUDO_PASSWORD 또는 SUDO_PASSWORD_COMMAND 설정이 필요합니다.');
    }
...
async function runPsqlCommand(query: string, options: { timeoutMs?: number }): Promise<CommandResult> {
  return runPtyProcess(
    'psql',
    [
      '-h',
...
const command: LegacyCommand = {
  ...
  async execute(message: Message) {
    ...
    try {
      ...
      await reply.edit(String(optionError instanceof Error ? optionError.message : optionError));
      return;
    }
    ...
    try {
      ...
      if (mode === 'bash') {
        if (/\bsudo\b/.test(commandPayload)) {
          ...
          if (!sudoPassword) {
            await reply.edit('sudo 명령을 실행하려면 SUDO_PASSWORD 또는 SUDO_PASSWORD_COMMAND가 필요합니다.');
            return;
          }
      ...
      logger.info(`Legacy exec command executed by ${message.author.tag} (${message.author.id}) in mode ${mode}`);
    } catch (error) {
      const err = error as Error;
      const messageToSend = maskSensitive(err?.message ? String(err.message) : String(error), secrets);
