...
const command: LegacyCommand = {
    ...
    async execute(message: Message) {
        ...
        await sentMessage.edit({
            components: [
                new TextDisplayBuilder().setContent(`🏓 퐁! 현재 봇 지연시간: ${latency}ms`),
                new SeparatorBuilder(),
                new TextDisplayBuilder().setContent(`API 지연시간: ${apiLatency}ms`),
                new SeparatorBuilder(),
                new TextDisplayBuilder().setContent(`웹소켓 상태: ${wsStatusString}`),
            ],
        });
    },
};
