import { Message, version as djsVersion, EmbedBuilder, Colors, WebSocketShardStatus } from 'discord.js';
        ...
        try {
            ...
            if (client.application) {
                try {
                    ...
                    logger.warn('Failed to fetch application commands for legacy status:', fetchError);
            ...
            logger.error('Full error object for legacy status:', JSON.stringify(err, Object.getOwnPropertyNames(err)));
            await message.reply({ content: `상태 정보를 가져오는 중 오류가 발생했습니다: ${err.message}`, allowedMentions: { parse: [] } });
        }
    },
};
