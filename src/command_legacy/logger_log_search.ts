...
import { 
    Message, PermissionsBitField, ButtonBuilder, ActionRowBuilder, 
    ButtonStyle, EmbedBuilder, MessageFlags, ComponentType,
    ChatInputCommandInteraction, MessageActionRowComponentBuilder,
    InteractionReplyOptions, InteractionEditReplyOptions, MessagePayload,
    ChannelType, TextDisplayBuilder, ThumbnailBuilder, SectionBuilder, SeparatorBuilder, FileBuilder, AttachmentBuilder, MessageComponentInteraction
} from 'discord.js';
            ...
            if (log.event_data && typeof log.event_data === 'object') {
                ...
                switch (log.event_type) {
                    ...
                    case 'guildMemberAdd': {
                        ...
                        if (memberTag) memberAddDetails.push(`**사용자:** ${memberTag} (${memberId || 'ID 없음'})`);
                        
                        const member = data.member as JsonData;
                        const user = data.user as JsonData;

                        const accountAge = num(member?.createdTimestamp) || num(user?.createdTimestamp);
                        if (accountAge) {
                            memberAddDetails.push(`**계정 생성일:** <t:${Math.floor(accountAge / 1000)}:R>`);
                        }
                    ...
                    case 'guildMemberRemove': {
                        ...
                        const memberTag = str(data.memberTag);
                        const memberId = str(data.memberId);
                        if (memberTag) memberRemoveDetails.push(`**사용자:** ${memberTag} (${memberId || 'ID 없음'})`);
                        ...
                        if (joinedAt) {
                            memberRemoveDetails.push(`**가입일:** <t:${Math.floor(joinedAt / 1000)}:R>`);
                        }
                    ...
                    case 'guildMemberUpdate': {
                         ...
                         if (oldNick !== newNick) {
                             memberUpdateDetails.push(`**닉네임:** ${oldNick || '(없음)'} -> ${newNick || '(없음)'}`);
                         }

                         // 역할 변경 확인
                         const oldRolesData = (data.oldMember as JsonData)?.roles;
                         const newRolesData = (data.newMember as JsonData)?.roles;
                         const oldRoles = Array.isArray(oldRolesData) ? (oldRolesData as unknown[]).filter(r => typeof r === 'string') : [];
                         const newRoles = Array.isArray(newRolesData) ? (newRolesData as unknown[]).filter(r => typeof r === 'string') : [];
                         
                         const addedRoles = newRoles.filter(r => !oldRoles.includes(r));
                         const removedRoles = oldRoles.filter(r => !newRoles.includes(r));

                         if (addedRoles.length > 0) memberUpdateDetails.push(`**역할 추가:** ${addedRoles.length}개`);
                         if (removedRoles.length > 0) memberUpdateDetails.push(`**역할 제거:** ${removedRoles.length}개`);

                         // 사용자 태그 fallback
                         const updatedMember = member ?? (log.user_id ? { id: log.user_id, tag: str(user?.tag, `<@${log.user_id}>`) } : null);

                         if (updatedMember) {
                             const tag = str((updatedMember).tag);
                             const id = str((updatedMember).id);
                             memberUpdateDetails.unshift(`**대상:** ${tag || '알 수 없음'} (${id || 'ID 없음'})`);
                         }
                         
                         eventSpecificsText = memberUpdateDetails.length > 0 ? memberUpdateDetails.join('\n') : '(내용 없음)';
                         break;
                    }
                    ...
                    case 'stickerCreate': {
                        ...
                        if (id) {
                            ...
                            const tagStr = Array.isArray(tags) ? tags.map(t => str(t)).join(', ') : str(tags);
                            ...
                            const formatType = num(sticker.format_type);
                            if (formatType) { // format_type can be 0, but usually 1-4
                                const formatTypes: Record<number, string> = { 1: 'PNG', 2: 'APNG', 3: 'LOTTIE', 4: 'GIF' };
                                stickerDetails.push(`**포맷:** ${formatTypes[formatType] ?? `알 수 없는 포맷 (${formatType})`}`);
                            }
                    ...
                    case 'stickerUpdate': {
                        ...
                        if (newId) {
                            stickerDetails.push(`**스티커:** ${str(newSticker.name, 'N/A')} (ID: ${newId})`);
                            if (oldSticker) {
                                const oldName = str(oldSticker.name);
                                const newName = str(newSticker.name);
                                if (oldName !== newName) stickerDetails.push(`**이름 변경:** \`\`\`${oldName}\`\`\` -> \`\`\`${newName}\`\`\``);
                                
                                const oldDesc = str(oldSticker.description);
                                const newDesc = str(newSticker.description);
                                if (oldDesc !== newDesc) stickerDetails.push(`**설명 변경:** \`\`\`${oldDesc || '(없음)'}\`\`\` -> \`\`\`${newDesc || '(없음)'}\`\`\``);
                                
                                const oldTags = oldSticker.tags;
                                const newTags = newSticker.tags;
                                const oldTagsStr = Array.isArray(oldTags) ? oldTags.map(t => str(t)).join(', ') : str(oldTags);
                                const newTagsStr = Array.isArray(newTags) ? newTags.map(t => str(t)).join(', ') : str(newTags);
                                
                                if (oldTagsStr !== newTagsStr) stickerDetails.push(`**태그 변경:** \`\`\`${oldTagsStr || '(없음)'}\`\`\` -> \`\`\`${newTagsStr || '(없음)'}\`\`\``);
                            } else {
                        ...
                        if (newId && num(newSticker.format_type) !== 3) {
                             try {
                                const fetchedSticker = await interaction.guild?.stickers.fetch(newId);
                                if (fetchedSticker?.url) {
                                   thumbnailComponent = new ThumbnailBuilder({ media: { url: fetchedSticker.url } });
                                }
                            } catch { /* empty */ }
                    ...
                    case 'emojiCreate': {
                        ...
                        if (id) {
                            ...
                            thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/emojis/${id}.${ext}?size=64` } });
                        } else if (log.user_id) {
                            try {
                                const executorUser = await interaction.client.users.fetch(log.user_id);
                                thumbnailComponent = new ThumbnailBuilder({ media: { url: executorUser.displayAvatarURL({ forceStatic: false, size: 64 }) } });
                    ...
                    case 'emojiUpdate': {
                        ...
                        if (newId) {
                            ...
                            emojiDetails.push(`**이모지:** ${newName} (ID: ${newId}) <:${newName}:${newId}>`);
                            if (oldEmoji) {
                                const oldName = str(oldEmoji.name);
                                if (oldName !== newName) {
                                    emojiDetails.push(`**이름 변경:** \`\`\`${oldName}\`\`\` -> \`\`\`${newName}\`\`\``);
                                }
                            } else {
                        ...
                        if (newId) {
                            ...
                            thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/emojis/${newId}.${ext}?size=64` } });
                        } else if (log.user_id) {
                            try {
                                const executorUser = await interaction.client.users.fetch(log.user_id);
                                thumbnailComponent = new ThumbnailBuilder({ media: { url: executorUser.displayAvatarURL({ forceStatic: false, size: 64 }) } });
                    ...
                    case 'guildBanAdd': {
                        ...
                        if (reason) {
                            const trimmedReason = reason.length > 200 ? `${reason.substring(0, 200)}...` : reason;
                    ...
                    case 'guildScheduledEventCreate': {
                        ...
                        if (event) {
                            ...
                            if (desc) eventDetails.push(`**설명:** ${desc.substring(0, 100)}${desc.length > 100 ? '...' : ''}`);
                            
                            const startTime = str(event.scheduledStartTime);
                            if (startTime) eventDetails.push(`**시작 시간:** <t:${Math.floor(new Date(startTime).getTime() / 1000)}:F>`);
                            const endTime = str(event.scheduledEndTime);
                            if (endTime) eventDetails.push(`**종료 시간:** <t:${Math.floor(new Date(endTime).getTime() / 1000)}:F>`);
                            
                            const entityType = num(event.entityType);
                            if (entityType !== undefined) {
                                const types: Record<number, string> = {1: '스테이지', 2: '음성 채널', 3: '외부 링크'};
                                eventDetails.push(`**유형:** ${types[entityType] || `알 수 없음 (${entityType})`}`);
                            }
                            ...
                            if (creatorId) {
                                ...
                                } catch { eventDetails.push(`**생성자 ID:** ${creatorId}`); }
                            }
                        }
                        ...
                        if (id && image) {
                            thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/guild-events/${id}/${image}.png?size=64` } });
                        } else if (creatorId) {
                            try {
                                const creator = await interaction.client.users.fetch(creatorId);
                    ...
                    case 'guildScheduledEventUpdate': {
                        ...
                        const newEvent = (data.newScheduledEvent ?? data.scheduledEvent) as JsonData;
                        const newId = str(newEvent?.id);

                        if (newId) {
                            eventDetails.push(`**이벤트:** ${str(newEvent.name, 'N/A')} (ID: ${newId})`);
                            if (oldEvent) {
                                const oldName = str(oldEvent.name);
                                const newName = str(newEvent.name);
                                if (oldName !== newName) eventDetails.push(`**이름 변경:** \`\`\`${oldName}\`\`\` -> \`\`\`${newName}\`\`\``);
                                
                                const oldDesc = str(oldEvent.description);
                                const newDesc = str(newEvent.description);
                                if (oldDesc !== newDesc) eventDetails.push(`**설명 변경:** \`\`\`${(oldDesc || '').substring(0,30)}...\`\`\` -> \`\`\`${(newDesc || '').substring(0,30)}...\`\`\``);
                                
                                const oldStart = str(oldEvent.scheduledStartTime);
                                const newStart = str(newEvent.scheduledStartTime);
                                if (oldStart && newStart && new Date(oldStart).getTime() !== new Date(newStart).getTime()) {
                                    eventDetails.push(`**시작 시간 변경:** <t:${Math.floor(new Date(oldStart).getTime() / 1000)}:R> -> <t:${Math.floor(new Date(newStart).getTime() / 1000)}:R>`);
                                }
                                ...
                                if (oldStatus !== newStatus) {
                                     ...
                                     eventDetails.push(`**상태 변경:** ${statuses[oldStatus] || `상태 ${oldStatus}`} -> ${statuses[newStatus] || `상태 ${newStatus}`}`);
                                }
                                ...
                                if (oldType !== newType) {
                                     const types: Record<number, string> = {1: '스테이지', 2: '음성 채널', 3: '외부 링크'};
                                     eventDetails.push(`**유형 변경:** ${types[oldType] || `타입 ${oldType}`} -> ${types[newType] || `타입 ${newType}`}`);
                                }
                            } else {
                        ...
                        if (newId && image) {
                            thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/guild-events/${newId}/${image}.png?size=64` } });
                        } else if (creatorId) {
                             try {
                                const creator = await interaction.client.users.fetch(creatorId);
                                thumbnailComponent = new ThumbnailBuilder({ media: { url: creator.displayAvatarURL({ forceStatic: false, size: 64 }) } });
                     ...
                     case 'guildScheduledEventUserAdd': {
                        ...
                        if (userId) {
                            try {
                             ...
                             thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/guild-events/${eventDataId}/${eventImage}.png?size=64` } });
                        }
                        break;
                    }
                    ...
                    case 'inviteCreate': {
                        ...
                        if (invite) {
                            ...
                            const inviterId = str(invite.inviterId) || str((invite.inviter as JsonData)?.id);
                            ...
                            const channelId = str(invite.channelId) || str((invite.channel as JsonData)?.id);
                            ...
                            if (maxUses !== undefined) inviteDetails.push(`**최대 사용:** ${maxUses === 0 ? '무제한' : maxUses}`);
                            
                            const maxAge = num(invite.maxAge);
                            if (maxAge !== undefined) inviteDetails.push(`**만료:** ${maxAge === 0 ? '없음' : `${maxAge / 60 / 60}시간`}`);
                            
                            const temporary = invite.temporary;
                            if (temporary !== undefined) inviteDetails.push(`**임시 멤버십:** ${temporary ? '예' : '아니오'}`);
                            
                            const createdAt = str(invite.createdAt);
                            if (createdAt) inviteDetails.push(`**생성일:** <t:${Math.floor(new Date(createdAt).getTime()/1000)}:R>`);
                        }
                        eventSpecificsText = inviteDetails.length > 0 ? inviteDetails.join('\n') : '(내용 없음)';
                        
                        const inviterIdForThumbnail = str(invite?.inviterId) || str((invite?.inviter as JsonData)?.id);
                    ...
                    case 'guildUpdate': {
                        ...
                        if (oldName !== newName) guildChanges.push(`**이름:** \`\`\`${oldName}\`\`\` -> \`\`\`${newName}\`\`\``);
                        
                        if (str(oldGuild.icon) !== str(newGuild.icon)) guildChanges.push(`**아이콘 변경됨**`);
                        if (str(oldGuild.splash) !== str(newGuild.splash)) guildChanges.push(`**초대 배경 변경됨**`);
                        if (str(oldGuild.discoverySplash) !== str(newGuild.discoverySplash)) guildChanges.push(`**탐색 스플래시 변경됨**`);
                        if (str(oldGuild.banner) !== str(newGuild.banner)) guildChanges.push(`**배너 변경됨**`);
                        
                        const oldOwnerId = str(oldGuild.ownerId);
                        const newOwnerId = str(newGuild.ownerId);
                        if (oldOwnerId !== newOwnerId) {
                            try {
                                const oldOwnerUser = oldOwnerId ? await interaction.client.users.fetch(oldOwnerId) : null;
                                const newOwnerUser = newOwnerId ? await interaction.client.users.fetch(newOwnerId) : null;
                                guildChanges.push(`**소유자:** ${oldOwnerUser?.tag ?? oldOwnerId} -> ${newOwnerUser?.tag ?? newOwnerId}`);
                            } catch { guildChanges.push(`**소유자 ID:** ${oldOwnerId} -> ${newOwnerId}`);}
                        }
                        
                        const oldAfk = str(oldGuild.afkChannelId);
                        const newAfk = str(newGuild.afkChannelId);
                        if (oldAfk !== newAfk) guildChanges.push(`**자리비움 채널:** ${oldAfk ? `<#${oldAfk}>` : '없음'} -> ${newAfk ? `<#${newAfk}>` : '없음'}`);
                        
                        const oldAfkTimeout = num(oldGuild.afkTimeout) || 0;
                        const newAfkTimeout = num(newGuild.afkTimeout) || 0;
                        if (oldAfkTimeout !== newAfkTimeout) guildChanges.push(`**자리비움 시간:** ${oldAfkTimeout/60}분 -> ${newAfkTimeout/60}분`);
                        
                        const oldSystem = str(oldGuild.systemChannelId);
                        const newSystem = str(newGuild.systemChannelId);
                        if (oldSystem !== newSystem) guildChanges.push(`**시스템 채널:** ${oldSystem ? `<#${oldSystem}>` : '없음'} -> ${newSystem ? `<#${newSystem}>` : '없음'}`);
                        
                        const oldVerif = num(oldGuild.verificationLevel);
                        const newVerif = num(newGuild.verificationLevel);
                        if (oldVerif !== newVerif) guildChanges.push(`**인증 수준 변경됨** (L${oldVerif} -> L${newVerif})`);
                        
                        const oldFilter = num(oldGuild.explicitContentFilter);
                        const newFilter = num(newGuild.explicitContentFilter);
                        if (oldFilter !== newFilter) guildChanges.push(`**콘텐츠 필터 변경됨** (L${oldFilter} -> L${newFilter})`);
                        
                        const oldMfa = num(oldGuild.mfaLevel);
                        const newMfa = num(newGuild.mfaLevel);
                        if (oldMfa !== newMfa) guildChanges.push(`**2FA 요구사항 변경됨** (L${oldMfa} -> L${newMfa})`);
                        
                        const oldVanity = str(oldGuild.vanityURLCode);
                        const newVanity = str(newGuild.vanityURLCode);
                        if (oldVanity !== newVanity) guildChanges.push(`** Vanity URL:** \`\`\`${oldVanity || '없음'}\`\`\` -> \`\`\`${newVanity || '없음'}\`\`\``);
                        
                        const oldDesc = str(oldGuild.description);
                        const newDesc = str(newGuild.description);
                        if (oldDesc !== newDesc) guildChanges.push(`**설명 변경:** \`\`\`${(oldDesc || '').substring(0,30)}...\`\`\` -> \`\`\`${(newDesc || '').substring(0,30)}...\`\`\``);
                        
                        const oldLocale = str(oldGuild.preferredLocale);
                        const newLocale = str(newGuild.preferredLocale);
                        if (oldLocale !== newLocale) guildChanges.push(`**기본 언어:** ${oldLocale} -> ${newLocale}`);
                        ...
                        if (guildForIcon?.iconURL()) {
                             ...
                             thumbnailComponent = new ThumbnailBuilder({ media: { url: `https://cdn.discordapp.com/icons/${newId}/${newIcon}.png?size=64` } });
                        } else if (log.user_id) { 
                            try {
                                const executor = await interaction.client.users.fetch(log.user_id);
                                thumbnailComponent = new ThumbnailBuilder({ media: { url: executor.displayAvatarURL({ forceStatic: false, size: 64 }) } });
                    ...
                    case 'channelCreate': {
                        ...
                        if (type !== undefined) details.push(`**타입:** ${ChannelType[type] ?? type}`);
                    ...
                    case 'channelUpdate': {
                        ...
                        if (newChannel) {
                            details.push(`**채널:** ${newName || 'N/A'} (<#${newId || 'ID 없음'}>)`);
                            if (oldChannel) {
                                const oldName = str(oldChannel.name);
                                if (oldName !== newName) details.push(`**이름 변경:** \`\`\`${oldName}\`\`\` -> \`\`\`${newName}\`\`\``);
                                
                                const oldTopic = str(oldChannel.topic);
                                const newTopic = str(newChannel.topic);
                                if (oldTopic !== newTopic) details.push(`**주제 변경:** \`\`\`${(oldTopic || '').substring(0,30)}...\`\`\` -> \`\`\`${(newTopic || '').substring(0,30)}...\`\`\``);
                                
                                const oldParent = str(oldChannel.parentId);
                                const newParent = str(newChannel.parentId);
                                if (oldParent !== newParent) details.push(`**카테고리 변경:** ${oldParent ? `<#${oldParent}>` : '없음'} -> ${newParent ? `<#${newParent}>` : '없음'}`);
                                
                                const oldType = num(oldChannel.type);
                                const newType = num(newChannel.type);
                                if (oldType !== newType) {
                                     details.push(`**타입 변경:** ${oldType !== undefined ? (ChannelType[oldType] ?? oldType) : 'unknown'} -> ${newType !== undefined ? (ChannelType[newType] ?? newType) : 'unknown'}`);
                                }
                                
                                const oldNsfw = oldChannel.nsfw;
                                const newNsfw = newChannel.nsfw;
                                if (oldNsfw !== newNsfw) details.push(`**NSFW:** ${oldNsfw ? '예':'아니오'} -> ${newNsfw ? '예':'아니오'}`);
                                
                                const oldRate = num(oldChannel.rateLimitPerUser);
                                const newRate = num(newChannel.rateLimitPerUser);
                                if (oldRate !== newRate) details.push(`**슬로우 모드:** ${oldRate || 0}초 -> ${newRate || 0}초`);
                    ...
                    case 'channelPinsUpdate': {
                        const details: string[] = [];
                        const channelId = str(data.channelId) || str((data.channel as JsonData)?.id);
                    ...
                    case 'threadCreate': {
                        ...
                        if (thread) {
                            details.push(`**스레드 이름:** ${name || 'N/A'} (<#${id || 'ID 없음'}>)`);
                            if (id) details.push(`**ID:** ${id}`);
                            ...
                            if (ownerId) {
                                ...
                                } catch { details.push(`**생성자 ID:** ${ownerId}`); }
                            }
                    ...
                    case 'threadUpdate': {
                        ...
                        if (newThread) {
                            details.push(`**스레드:** ${newName || 'N/A'} (<#${newId || 'ID 없음'}>)`);
                            if (oldThread) {
                                ...
                                if (oldArchived !== newArchived) details.push(`**보관 상태:** ${oldArchived ? '보관됨':'활성'} -> ${newArchived ? '보관됨':'활성'}`);
                                
                                const oldLocked = oldThread.locked;
                                const newLocked = newThread.locked;
                                if (oldLocked !== newLocked) details.push(`**잠금 상태:** ${oldLocked ? '잠김':'해제'} -> ${newLocked ? '잠김':'해제'}`);
                                
                                const oldDur = num(oldThread.autoArchiveDuration);
                                const newDur = num(newThread.autoArchiveDuration);
                                if (oldDur !== newDur) details.push(`**자동 보관 변경:** ${oldDur ? oldDur/60 : 0}분 -> ${newDur ? newDur/60 : 0}분`);
                    ...
                    case 'userUpdate': {
                        ...
                        if (newUser) {
                            ...
                            if (oldUser) {
                                ...
                                if (oldUsername !== newUsername) details.push(`**사용자명 변경:** \`\`\`${oldUsername}\`\`\` -> \`\`\`${newUsername}\`\`\``);
                                
                                const oldDisc = str(oldUser.discriminator);
                                const newDisc = str(newUser.discriminator);
                                if (oldDisc !== newDisc) details.push(`**태그 변경:** #${oldDisc} -> #${newDisc}`);
                                
                                if (str(oldUser.avatar) !== str(newUser.avatar)) details.push(`**아바타 변경됨**`);
                                
                                const oldGlobal = str(oldUser.globalName);
                                const newGlobal = str(newUser.globalName);
                                if (oldGlobal !== newGlobal) details.push(`**표시 이름 변경:** \`\`\`${oldGlobal || '(없음)'}\`\`\` -> \`\`\`${newGlobal || '(없음)'}\`\`\``);
                            }
                        } else {
                    ...
                    case 'voiceStateUpdate': {
                        ...
                        const member = (data.member ?? (newState?.member) ?? (oldState?.member)) as JsonData;
            ...
            if (eventData && Array.isArray(eventData.attachments)) {
                for (const attachmentData of eventData.attachments) {
                    if (attachmentData.webdavPath && attachmentData.filename) {
                        ...
                        try {
                            nasFileBuffer = await storageManager.download(attachmentData.webdavPath);
                        } catch {
                            // Suppress error if file not found or download failed
                        }
                        if (nasFileBuffer) {
                            const uniqueAttachmentFilename = `${log.id}_${attachmentData.id}_${attachmentData.filename}`.replace(/[^a-zA-Z0-9_.-]/g, '_');
...
const command: LegacyCommand = {
    ...
    async execute(message: Message) {
        ...
        if (startDateString) {
            ...
            if (!parsed) {
                await message.reply({ content: '오류: 유효하지 않은 시작 날짜 형식입니다. YYYY-MM-DD 형식으로 입력해주세요.', embeds: [], components: [] }); return;
            }
            startDate = parsed;
        }
        if (endDateString) {
            ...
            if (!parsed) {
                await message.reply({ content: '오류: 유효하지 않은 종료 날짜 형식입니다. YYYY-MM-DD 형식으로 입력해주세요.', embeds: [], components: [] }); return;
            }
            endDate = parsed;
        }
        if (startDate && endDate && startDate.getTime() > endDate.getTime()) {
            await message.reply({ content: '오류: 검색 시작일이 종료일보다 늦을 수 없습니다.', embeds: [], components: [] }); return;
        }
        ...
        const noOptionsProvidedInitially = !userId && !channelId && !startDateString && !endDateString && !eventType;
        ...
        const pseudoInteraction = {
            ...
            editReply: (options: string | MessagePayload | InteractionEditReplyOptions) => reply.edit(options)
        ...
        try {
            ...
            collector.on('collect', (i) => {
                ...
                if (i.customId === 'log_search_close') {
                    ...
                    await reply.delete().catch(() => { /* empty */ });
                    collector.stop();
                    return;
                }
