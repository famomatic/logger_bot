...
export class WebDAVProvider implements StorageProvider {
  ...
  constructor(url: string, username?: string, password?: string, basePath = '/') {
    ...
    if (username && password) {
        ...
        const basicAuthHeader = Buffer.from(`${username}:${password}`, 'utf8').toString('base64');
        options.headers = {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries) {
                ...
                logger.warn(`Failed to upload file to WebDAV path ${fullPath}. Retrying after ${delay}ms... (${attempt}/${maxRetries})`, error);
                await new Promise(resolve => setTimeout(resolve, delay));
            } else {
                logger.error(`Failed to upload file to WebDAV path ${fullPath} after ${maxRetries} attempts:`, error);
                throw error;
            }
        }
    }
