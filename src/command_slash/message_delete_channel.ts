import { SlashCommandBuilder, CommandInteraction, PermissionsBitField, GuildTextBasedChannel, Collection, Message, Client, ChannelType, SlashCommandOptionsOnlyBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ComponentType, MessageFlags } from 'discord.js';
    ...
    async execute(interaction: CommandInteraction, client: Client) {
        ...
        try {
            ...
            if (!fetchedChannel) {
                ...
                await interaction.reply({ content: `오류: ID가 ${targetChannelId}인 채널을 찾을 수 없습니다.`, flags: MessageFlags.Ephemeral });
                return;
            }
            if (!fetchedChannel.isTextBased() || fetchedChannel.isDMBased() || !('guild' in fetchedChannel) ) { // Check if it's a guild text-based channel
                 logger.warn(`${logPrefix} Channel ${targetChannelId} is not a valid guild text-based channel (Type: ${fetchedChannel.type}).`);
                await interaction.reply({ content: `오류: ID가 ${targetChannelId}인 채널은 유효한 서버 텍스트 채널이 아닙니다. (채널 타입: ${ChannelType[fetchedChannel.type]})`, flags: MessageFlags.Ephemeral });
                return;
            }
            ...
            await interaction.reply({ content: `오류: 채널 ID ${targetChannelId}를 가져오는 중 오류가 발생했습니다. ID를 확인해주세요.`, flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        */
        ...
        if (!botPermissionsInChannel?.has(PermissionsBitField.Flags.ReadMessageHistory) || !botPermissionsInChannel?.has(PermissionsBitField.Flags.ManageMessages)) {
            logger.warn(`${logPrefix} Missing permissions (ReadMessageHistory or ManageMessages) in channel ${targetChannel.id} for guild ${interaction.guildId}.`);
            await interaction.reply({ content: `오류: 채널 #${targetChannel.name}에서 메시지를 읽거나 삭제할 권한이 없습니다. 봇의 권한을 확인해주세요.`, flags: MessageFlags.Ephemeral });
            return;
        }
        if (!targetChannel.viewable) {
            logger.warn(`${logPrefix} Channel ${targetChannel.id} is not viewable by the bot.`);
            await interaction.reply({ content: `오류: 채널 #${targetChannel.name}에 접근할 수 없습니다. 봇의 채널 보기 권한을 확인해주세요.`, flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        try {
            const collectorFilter = (i: import('discord.js').MessageComponentInteraction) => i.user.id === interaction.user.id;
            ...
            if (confirmation.customId === confirmButtonId) {
                await confirmation.update({ content: `⏳ 채널 #${targetChannel.name}(${targetChannel.id}) 메시지 삭제를 시작합니다... (최종 결과는 새 메시지로 전송됩니다)`, components: [] });
                logger.info(`${logPrefix} CONFIRMED via button: Initiating CHANNEL-WIDE message deletion for channel ${targetChannel.id} by ${interaction.user.tag} (${interaction.user.id})`);
            } else if (confirmation.customId === cancelButtonId) {
                await confirmation.update({ content: '✅ 작업이 취소되었습니다.', components: [] });
                logger.info(`${logPrefix} Channel-wide deletion cancelled by user ${interaction.user.tag} via button.`);
                return;
            }
        } catch {
            logger.warn(`${logPrefix} Channel-wide deletion confirmation timed out for user ${interaction.user.tag}.`);
        ...
        try {
            logger.info(`${logPrefix} Starting deletion for channel #${targetChannel.name} (${targetChannel.id})`);
            ...
            while (fetchMore) {
                try {
                    const messages: Collection<string, Message> = await targetChannel.messages.fetch({ limit: 100, before: lastMessageId });
                    ...
                    if (messagesToDelete.size > 0) {
                        try {
                            const deletedResult = await targetChannel.bulkDelete(messagesToDelete, true);
                            channelDeletedCount += deletedResult.size;
                            logger.debug(`${logPrefix} Bulk deleted ${deletedResult.size} messages.`);
                            await new Promise(resolve => setTimeout(resolve, 1100));
                        } catch (error) {
                            const bulkDeleteError = error as Error;
                            logger.error(`${logPrefix} Failed to bulk delete messages:`, bulkDeleteError);
                    ...
                    if (oldMessagesToDelete.length > 0) {
                        logger.debug(`${logPrefix} Deleting ${oldMessagesToDelete.length} old messages individually...`);
                            ...
                            while (retries < 3 && !deletedSuccessfully) {
                                try {
                                    ...
                                    if (deleteError.status === 429) {
                                        ...
                                        logger.warn(`${logPrefix} Rate limited deleting old message ${message.id}. Retrying after ${retryAfter}ms... (${retries}/3)`);
                                        await interaction.followUp({ content: `API 제한, 채널 #${targetChannel.name} 오래된 메시지 삭제 지연. ${retryAfter / 1000}초 후 재시도... (${retries}/3)`, flags: MessageFlags.Ephemeral }).catch(() => { /* empty */ });
                                        await new Promise(resolve => setTimeout(resolve, retryAfter + 500));
                                    } else {
                                        logger.warn(`${logPrefix} Failed to delete old message ${message.id}: ${deleteError.message} (Code: ${deleteError.code})`);
            ...
            await channelToSendResponse.send(finalReport).catch((sendError: unknown) => { logger.error(`${logPrefix} Failed to send final CHANNEL deletion report:`, sendError); });
            logger.info(`${logPrefix} Finished CHANNEL-WIDE message deletion for channel ${targetChannel.id}. Deleted ~${totalDeletedCount} messages with ${totalErrorMessages.length} errors in ${duration}s.`);

        } catch (error) {
            const err = error as Error;
            logger.error(`${logPrefix} Critical error during CHANNEL-WIDE message deletion process for channel ${targetChannel.id}:`, err);
            const criticalErrorMessage = `${interaction.user.toString()}님, 채널 #${targetChannel.name} 메시지 삭제 처리 중 심각한 오류가 발생했습니다: ${err.message}`;
            if (channelToSendResponse) { await channelToSendResponse.send(criticalErrorMessage).catch((sendError: unknown) => { logger.error(`${logPrefix} Failed to send critical error report for CHANNEL deletion:`, sendError); }); }
        }
    },
}; 