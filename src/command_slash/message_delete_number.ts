import { SlashCommandBuilder, CommandInteraction, PermissionsBitField, Client, GuildTextBasedChannel, Collection, Message, SlashCommandOptionsOnlyBuilder, MessageFlags } from 'discord.js';
...
export const command: SlashCommand = {
    ...
    async execute(interaction: CommandInteraction, client: Client) {
        ...
        if (amount <= 0) {
            await interaction.reply({ content: '삭제할 메시지 수는 1 이상이어야 합니다.', flags: MessageFlags.Ephemeral });
            return;
        }
        if (amount > 100) {
            await interaction.reply({ content: '한 번에 최대 100개의 메시지만 삭제할 수 있습니다.', flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        try {
            ...
            if (!fetched || !fetched.isTextBased() || fetched.isDMBased() || !("guild" in fetched)) {
                await interaction.editReply(`오류: ID가 ${channelId}인 유효한 서버 채널을 찾을 수 없습니다.`);
                return;
            }
        ...
        if (!botPerms?.has(PermissionsBitField.Flags.ReadMessageHistory) || !botPerms?.has(PermissionsBitField.Flags.ManageMessages)) {
            await interaction.editReply(`오류: 채널 #${channel.name}에서 메시지를 삭제할 권한이 없습니다.`);
            return;
        }
        ...
        try {
            ...
            await interaction.editReply(`✅ 채널 #${channel.name}에서 ${deletedCount}개의 메시지를 삭제했습니다.`);
        } catch (err) {
            const error = err instanceof Error ? err : new Error(String(err));
