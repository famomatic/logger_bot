...
export const command: SlashCommand = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('userid').setDescription('메시지를 받을 사용자의 ID').setRequired(true),
        )
        .addStringOption((option) =>
    ...
    async execute(interaction: CommandInteraction, client: Client) {
        ...
        try {
            ...
            if (!/^\d{17,19}$/.test(targetUserId)) {
                await interaction.editReply({
                    content: `오류: 제공된 사용자 ID (${targetUserId})가 올바른 형식이 아닙니다.`,
                });
                return;
            }
            ...
            await interaction.editReply({
                content: `오류: 사용자 ID ${targetUserId}를 찾을 수 없습니다.`,
            });
            return;
        }
        ...
        try {
            ...
            if (err.code === 50007) {
                ...
                await interaction.editReply({
                    content: `❌ 오류: ${targetUser.tag} (${targetUserId})님에게 DM을 보낼 수 없습니다. (DM 비활성화 또는 봇 차단)`,
                });
            } else {
                await interaction.editReply({
