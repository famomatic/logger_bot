import { SlashCommandBuilder, CommandInteraction, PermissionsBitField, GuildTextBasedChannel, Collection, Message, Client, SlashCommandOptionsOnlyBuilder, MessageFlags, Attachment, Guild } from 'discord.js';
...
// --- 타입 정의 수정 ---
interface LegacyCommand { // 레거시 커맨드 타입 정의가 여기에 필요할 수 있음
    name: string;
    execute: (message: Message) => Promise<void>;
}
interface SlashCommand {
    data: SlashCommandOptionsOnlyBuilder | Omit<SlashCommandBuilder, "addSubcommand" | "addSubcommandGroup">;
    execute: (interaction: CommandInteraction, client: Client & { legacyCommands?: Collection<string, LegacyCommand> }) => Promise<void>; // client 타입에 legacyCommands 추가
}
...
export const command: SlashCommand = {
    ...
    async execute(interaction: CommandInteraction, client: Client & { legacyCommands?: Collection<string, LegacyCommand> }) {
        ...
        logger.info(`Initiating bulk message logging for guild ${targetGuildId} by ${interaction.user.tag} (${interaction.user.id})`);
        ...
        try {
            ...
            await interaction.editReply(`오류: 대상 서버 ID(${targetGuildId})를 찾을 수 없거나 봇이 해당 서버에 없습니다.`);
            return;
        }
        // --------------------------
        ...
        const channelStats: Record<string, { processed: number; newlyLogged: number; name: string }> = {}; // 채널별 통계 저장용 객체 (로그용)
        ...
        try {
            ...
            if (totalChannelsToProcess === 0) {
                ...
                await interaction.editReply(`오류: 이 서버에서 메시지 기록을 읽을 수 있는 채널을 찾을 수 없습니다. (봇 권한 확인 필요)`);
                return;
            }

            logger.info(`Found ${totalChannelsToProcess} accessible text channels in guild ${targetGuildId} to process.`);
            await interaction.editReply(`길드 '${guild.name}' (${targetGuildId}) 내 ${totalChannelsToProcess}개 채널의 모든 메시지 확인/시작합니다... (매우 오래 걸릴 수 있습니다)`);
            ...
            for (const channel of channels.values()) {
                ...
                while (fetchMore) {
                    try {
                        ...
                        if (messages.size > 0) {
                            logger.debug(`Fetched ${messages.size} messages in batch for channel ${channel.id}`);
                            ...
                            for (const message of messages.values()) {
                                ...
                                const isAuthorDeveloper = config.getDevLevel(message.author.id) >= 1;
                                let isLegacyCommandByDev = false;
                                if (isAuthorDeveloper && legacyCommandPrefixes.length > 0) {
                                    const matchedPrefix = legacyCommandPrefixes.find(prefix => message.content.startsWith(prefix));
                                    if (matchedPrefix) {
                                        const isExactMatch = message.content === matchedPrefix;
                                        const isFollowedBySpace = message.content.startsWith(`${matchedPrefix} `);
                                ...
                                if (isLegacyCommandByDev) {
                                    logger.debug(`Skipping logging legacy command message ${message.id} by dev ${message.author.tag}`);
                                    continue;
                                }
                                ...
                                const task = (async () => {
                                    try {
                                            ...
                                            for (const attachment of message.attachments.values()) {
                                                ...
                                                if (config.storage.type) {
                                                    try {
                                                        ...
                                                        const relativePath = createWebDAVPath(guild.id, message.channel.id, message.id, attachment);
                                        ...
                                        logger.error(`Failed to log/check message ${message.id} from channel ${channel.id}:`, logError);
                                        errorCount++;
                                    }
                                })();
                                tasks.push(task);
                ...
                logger.info(`Finished processing channel ${channel.id} (${channel.name}). Checked ${channelProcessedCount} messages, newly logged ${channelNewlyLoggedCount}. Took ${((channelEndTime - channelStartTime)/1000).toFixed(2)}s.`);
            }
            ...
            if (durationInSeconds > 900) {
                logger.info(`Bulk logging took longer than 15 minutes (${durationInSeconds}s). Sending result as a normal message.`);
                try {
                    await interaction.channel?.send(`${interaction.user.toString()} ${finalReply}`);
                    // deferReply에 대한 응답도 필요함
                    await interaction.editReply({ content: "✅ 작업이 완료되었으나, 15분이 초과되어 현재 채널에 일반 메시지로 결과를 전송했습니다." });
                } catch (sendError) {
                    logger.error("Failed to send normal message or edit initial reply for long-running bulk logging result:", sendError);
                    // 최후의 수단으로 followUp 시도 (이것도 실패할 가능성 높음)
                    try {
                        await interaction.followUp({content: "작업은 완료되었으나, 결과 메시지 전송에 실패했습니다. 봇 로그를 확인해주세요.", flags: MessageFlags.Ephemeral});
                    } catch (finalFallbackError) {
                        logger.error("Failed to send even a fallback follow-up message:", finalFallbackError);
                    }
                }
            } else {
                try {
                    ...
                    logger.error("Failed to send detailed follow-up message for bulk logging result (under 15 mins):", followUpError);
                    // 15분 미만이어도 followUp 실패 시 일반 메시지로 시도
                    try {
                        await interaction.channel?.send(`${interaction.user.toString()} ${finalReply}\n\n⚠️ *자동 응답에 실패하여 일반 메시지로 전송합니다.*`);
                        await interaction.editReply({ content: "✅ 작업이 완료되었으나, 자동 응답에 실패하여 현재 채널에 일반 메시지로 결과를 전송했습니다." });
                    } catch (fallbackSendError) {
                         logger.error("Failed to send normal message as fallback for followUp failure (under 15 mins):", fallbackSendError);
                    }
                }
            }

            logger.info(`Finished bulk message logging check for guild ${targetGuildId}. Processed ${totalChannelsToProcess} channels, checked ${processedCount} messages (non-bot), newly logged ${newlyLoggedCount} from approx ${totalUniqueUsers} unique users with ${errorCount} errors in ${duration}s.`);
            ...
            try {
                ...
                logger.error("Failed to send follow-up error message after critical error during logging check:", followUpError);
            }
        }
    },
}; 