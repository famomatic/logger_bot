...
export const command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('guild_id').setDescription('제거할 길드 ID').setRequired(true))
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (devLevel < 3 && !isAdmin) {
      await interaction.reply({ content: '이 명령어는 관리자 또는 레벨3 개발자만 사용할 수 있습니다.', flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    await interaction.reply({ content: `✅ 길드 ${gid} 이(가) 승인 목록에서 제거되었습니다.`, flags: MessageFlags.Ephemeral });
  }
};
