...
export const command = {
  data: new SlashCommandBuilder()
    ...
    .addChannelOption(o => o.setName('channel').setDescription('알림을 보낼 채널').setRequired(true)
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!added) {
      await interaction.reply({ content: '유효하지 않은 이벤트 카테고리입니다.', flags: MessageFlags.Ephemeral });
      return;
    }
    logger.info(`/log-alert-add set by ${interaction.user.tag} in guild ${interaction.guildId} for category ${category} channel ${channel.id}`);
    await interaction.reply({ content: `이제 ${category} 이벤트 알림이 ${channel.toString()} 채널에 전송됩니다.`, flags: MessageFlags.Ephemeral });
  }
};
