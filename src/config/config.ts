...
function loadConfig() {
  ...
  return {
    ...
    // 통합 스토리지 설정
    storage: {
        type: (process.env.STORAGE_TYPE ?? (webdavHost ? 'webdav' : 'local')).toLowerCase() as 'webdav' | 's3' | 'smb' | 'local',
        local: {
    ...
    execCommandTimeoutMs: parseDurationToMs(process.env.EXEC_COMMAND_TIMEOUT ?? process.env.EXEC_COMMAND_TIMEOUT_MS),
  };
}
...
export function reloadConfig(): void {
  ...
  if (config.storage.webdav.enabled && config.storage.webdav.url) {
      console.log(chalk.green(`WebDAV storage enabled: ${config.storage.webdav.url}${config.storage.webdav.basePath}`));
  } else {
