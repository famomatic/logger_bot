...
export async function authorizeGuildId(guildId: string): Promise<void> {
  try {
    await pool.query('INSERT INTO authorized_guilds (guild_id) VALUES ($1) ON CONFLICT (guild_id) DO NOTHING', [guildId]);
...
export async function addAlertSubscription(guildId: string, category: string, channelId: string): Promise<void> {
  try {
    await pool.query(
...
export async function removeAlertSubscription(guildId: string, category: string, channelId: string): Promise<void> {
  try {
    await pool.query(
...
export async function fetchAlertSubscriptions(): Promise<AlertSubscriptionRow[]> {
  try {
    const res = await pool.query<AlertSubscriptionRow>('SELECT guild_id, channel_id, category FROM alert_subscriptions');
    return res.rows;
  } catch (error) {
...
export async function logEvent(
  ...
  if (!targetId) {
    logger.warn(`Attempted to log event ${eventType} for guild ${guildId} without a targetId. Skipping log.`);
  ...
  try {
    ...
    if (inserted) {
      void dispatchAlert(eventType, guildId, userId, channelId, targetId, data, timestamp, discordClient);
    }
    return inserted;
  } catch (error: unknown) {
    ...
    if (pgErr.code === '23514' && !isRetry) {
      logger.warn(`Partition not found for guild ${guildId} (error code ${pgErr.code}). Attempting to create it.`);
      ...
      try {
        ...
        logger.info(`Successfully created partition "${partitionTableName}" for guild ${guildId}. Retrying logEvent.`);
        // Retry the original logEvent call, marking it as a retry
        return await logEvent(eventType, guildId, userId, channelId, targetId, data, timestamp, true);
        ...
        if (pcErr.code === '42P07') {
          try {
            ...
            logger.info(`Attached existing partition "${partitionTableName}" for guild ${guildId}. Retrying logEvent.`);
            return await logEvent(eventType, guildId, userId, channelId, targetId, data, timestamp, true);
          } catch (attachError: unknown) {
            const aErr = attachError as PgError;
            if (aErr.code === '42710' || aErr.code === '42809') {
              logger.warn(`Partition "${partitionTableName}" already attached for guild ${guildId}. Continuing.`);
              return await logEvent(eventType, guildId, userId, channelId, targetId, data, timestamp, true);
            }
            logger.error(`Failed to attach existing partition for guild ${guildId}:`, attachError);
          }
        }
        logger.error(`Failed to create partition for guild ${guildId} after error ${pgErr.code}. Partition creation error:`, partitionCreateError);
      ...
      if (isRetry && pgErr.code === '23514') {
        logger.error(`Still failed to insert into partition for guild ${guildId} even after attempting to create it. Original error:`, error);
      }
      logOriginalError(error, eventType, targetId, guildId);
      return false; 
    }
...
function logOriginalError(error: unknown, eventType: string, targetId: string | null, guildId: string) {
    ...
    if (error instanceof Error) {
        ...
        try {
            logger.error(errorMessage + ` (Non-standard error object: type=${typeof error})`, JSON.stringify(error));
        } catch {
 ...
 */
export async function fetchLogs(guildId: string, page = 1, limit = 10, filters: { eventType?: string; userId?: string; channelId?: string; targetId?: string } = {}): Promise<{ logs: FetchedLogRow[]; totalPages: number; totalLogs: number }> {
...
export async function migrate() {
  ...
  try {
    ...
    `);
     ...
     await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_guild_timestamp ON event_logs (guild_id, "timestamp" DESC);`);
     await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_guild_event_type ON event_logs (guild_id, event_type);`);
     await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_guild_user_id ON event_logs (guild_id, user_id);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_guild_target_id ON event_logs (guild_id, target_id);`);

    // JSONB 검색 최적화를 위한 인덱스
    await client.query(`CREATE EXTENSION IF NOT EXISTS pg_trgm;`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_content_gin ON event_logs USING GIN ((data->>'content') gin_trgm_ops);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_newcontent_gin ON event_logs USING GIN ((data->>'newContent') gin_trgm_ops);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_event_logs_oldcontent_gin ON event_logs USING GIN ((data->>'oldContent') gin_trgm_ops);`);
...
export async function countLogs(guildId: string, filters: { eventType?: string; userId?: string } = {}): Promise<number> {
...
export async function getGuildLogStats(guildId: string): Promise<GuildLogStats> {
  ...
  try {
    ...
    const empty: GuildLogStatsRow = { total_logs: '0', message_create_count: '0', text_message_count: '0', total_text_characters: '0', attachment_count: '0', sticker_count: '0' };
    const r = row ?? empty;
    return {
 ...
 */
export async function searchLogs(params: SearchLogsParams): Promise<{ logs: LogEntry[], totalCount: number }> {
    const {
        guildId,
        keyword,
        userId,
    ...
    try {
        ...
        return {
            logs: logResult.rows.map(row => ({ // 반환되는 로그의 event_data와 timestamp 타입 일관성 유지
                ...row,
