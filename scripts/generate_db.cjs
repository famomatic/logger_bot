...
async function setupDatabase() {
  ...
  try {
    ...
    const dbExists = await client.query('SELECT 1 FROM pg_database WHERE datname = $1', [botDbName]);
    ...
    await botDbClient.query(`GRANT ALL PRIVILEGES ON DATABASE "${botDbName}" TO "${botDbUser}"`);
